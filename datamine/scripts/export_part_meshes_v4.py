...
OUT = '../site/public/meshes3_v4'
TEXOUT = '../site/public/tex3_v4'
...
_dup = glob.glob(BASE + 'duplicateassetisolation_assets_all_*.bundle')[0]
env = UnityPy.load(BASE + 'walker_assets_all.bundle', _dup)
objs = {}
objs_by_name = {}   # (cab_name, path_id) -> obj, for cross-bundle texture resolution
...
# ---- layered Shader-Graph bake (v4): for materials with no simple base map but
# tiled _DetailColorMap layers, composite base×detail(mask) into one albedo swatch
# and register it as a texture slot, so those triangles stop falling back to flat. ----
RES_BAKE = 256
...
def resolve_img(matco, ptr):
    """resolve a texture PPtr across bundles via the material's externals (m_FileID->CAB)."""
...
def bake_layered(matco, mt):
    """Composite the layered albedo to a flat RES_BAKE swatch; None if not layered."""
...
def register_baked(matco, img):
    """register a baked albedo image as a new texture slot (keyed by material)."""
        ...
        for s in ('_BaseMap', '_MainTex', '_BaseColorMap'):
            ...
            if te and te.get('m_Texture', {}).get('m_PathID'):
                ...
                if texco and texco.type.name == 'Texture2D':
                    ...
                    break
        # v4: no simple base map -> bake the layered Shader-Graph albedo instead of
        # falling back to a flat colour (that's what left crew/reactor parts white).
        if slot is None:
            baked = bake_layered(matco, mt)
            if baked is not None:
                slot = register_baked(matco, baked)
        cols = {}
...
def collect(tfk, M, lods):
    ...
    if mf and mf.get('m_PathID'):
        ...
        # v4 fix: multi-cell parts repeat identical sub-pieces (e.g. two 1x1
        # compartments). Key by name AND world position so repeated instances
        # don't clobber each other (was dropping the 2nd frame). LODs of one
        # instance share a position, so they still group correctly.
...
json.dump(index, open('../site/src/data/mesh_index_v4.json', 'w'), separators=(',', ':'))
